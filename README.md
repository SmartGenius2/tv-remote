...
adb connect 192.168.20.110:5555          # IP de tu TV
.\start-tv-remote.ps1 -Tv 192.168.20.110:5555
```
...
Sin el script: `python server.py --tv 192.168.20.110:5555 [--port 8080] [--lan]`.
...
- Por defecto solo escucha en `127.0.0.1` y comprueba que el `Origin` sea el mismo equipo.
...
```powershell
pip install -r requirements.txt
adb connect 192.168.20.110:5555          # your TV's IP
.\start-tv-remote.ps1 -Tv 192.168.20.110:5555
```
...
Without the script: `python server.py --tv 192.168.20.110:5555 [--port 8080] [--lan]`.
...
- By default it only listens on `127.0.0.1` and checks that the request `Origin` is the same host.
